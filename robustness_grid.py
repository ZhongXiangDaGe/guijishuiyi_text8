...
def build_model(name="resnet18", num_classes=100):
    if name == "resnet18":
...
def attack_prune(state_dict, device, amount=0.3, num_classes=100, model_name="resnet18", prune_mode="global_unstructured"):
...
def attack_noise(state_dict, device, sigma=1e-3, seed=0, num_classes=100, model_name="resnet18"):
    torch.manual_seed(seed)
    model = build_model(model_name, num_classes).to(device)
    model.load_state_dict(state_dict, strict=False)
    with torch.no_grad():
...
def attack_quantize(state_dict, device, n_bits=8, num_classes=100, model_name="resnet18"):
    model = build_model(model_name, num_classes).to(device)
    model.load_state_dict(state_dict, strict=False)
    with torch.no_grad():
        for _, p in model.named_parameters():
...
def run_grid(
    ...
    with open(outcsv, "w", newline="") as f:
        ...
        for shard_key, entry in iter_shards(ckpts):
            ...
            model_base = build_model(model_name, num_classes).to(device_t)
            ...
            for seed in seeds:
                ...
                for sigma in noise_sigmas:
                    model_after = attack_noise(state, device_t, sigma=sigma, seed=seed, num_classes=num_classes, model_name=model_name)
                    det_after, ber = det_and_ber(model_after, positions, bits_arr)
                    acc_after = accuracy_on_loader(model_after, test_loader, device_t)
                    writer.writerow(
                        [
                ...
                for nb in quant_bits:
                    model_after = attack_quantize(state, device_t, n_bits=nb, num_classes=num_classes, model_name=model_name)
                    det_after, ber = det_and_ber(model_after, positions, bits_arr)
                    acc_after = accuracy_on_loader(model_after, test_loader, device_t)
                    writer.writerow(
                        [
